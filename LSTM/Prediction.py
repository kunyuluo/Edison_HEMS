...
# Load the model
# *************************************************************************
with open('models/model_lstm_uni.pkl', 'rb') as f:
    model = pickle.load(f)

# Prepare the data
# *************************************************************************
file_path = '../Data/Edison_Oct2Mar_new.csv'
features_name = ['vfd_ac_line_power']
...
# *************************************************************************
# walk_forward = prediction.walk_forward_validation(rolling_size, 0)
# predict_values = walk_forward[0]
# actual_values = walk_forward[1]
...
print('LSTM Model\'s smape is: {}%'.format(round(evals.smape()*100, 1)))
print('LSTM Model\'s ope is: {}%'.format(round(evals.ope()*100, 1)))
print('LSTM Model\'s rmse is: {}'.format(round(evals.rmse(), 1)))
print('LSTM Model\'s cv is: {}%'.format(round(evals.cv()*100, 1)))
